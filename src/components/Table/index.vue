  ...
  <div class="table-component">
    ...
    >
      ...
      <template #bodyCell="{ column, text, index, record }">
        ...
        <template v-if="column.key === 'apiMenu'">
          <span>{{ $t(text) }}</span>
        </template>
        <!-- 函数式写法自定义 操作列 -->
        <template v-if="column.key === 'action'">
          <template v-for="(action, index) in getActions" :key="`${index}-${action.label}`">
            ...
            >
              <a @click.prevent="() => {}" :type="action.type">{{ $t(action.label) }}</a>
            </a-popconfirm>
            <span v-else-if="!action.permission">——</span>
            <!-- 按钮 -->
            <a v-else @click="action?.onClick(record)" :type="action.type">{{ $t(action.label) }}</a>
            <!-- 分割线 -->
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      console.log('props.column:',props.columns, props.columns.value);

      const columnsI18n = computed(() => props.columns.map(item => {
        return {
          ...item,
          title: item.dataIndex ? t(item.dataIndex) : t(item.key)
        }
      }))
      ...
      const pagination = computed(() => ({
        ...
        showTotal: () => h('span', {}, `${t('total')} ${total.value} ${t('records')}`),
      }));
      ...
      watch(() => locale, (val, old) => {
        console.log('table locale', val, old)
      })
